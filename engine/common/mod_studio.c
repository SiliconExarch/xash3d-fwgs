...
void Mod_StudioBigEndian( model_t *model, byte *buffer )
{
	...
	for( j = 0; j < phdr->numseq; j++ )
	{
		...
		if( pseqdesc->seqgroup )
		{
			...
			if( paSequences == NULL )
			{
				paSequences = (cache_user_t *)Mem_Malloc( com_studiocache, MAXSTUDIOGROUPS * sizeof( cache_user_t ));
			...
			if( !paSequences[pseqdesc->seqgroup].data )
			{
				...
				COM_ExtractFilePath( model->name, modelpath );
				...
				paSequences[pseqdesc->seqgroup].data = Mem_Malloc( com_studiocache, filesize );
				memcpy( paSequences[pseqdesc->seqgroup].data, buf, (size_t)filesize );
				Mem_Free( buf );
			}
...
#undef TRYSWAP
	...
	for( j = 0; j < phdr->numseqgroups; j++ )
	{
		...
		SWAP_INTS(mstudioseqgroup_t,unused2);
	}
	...
	for( j = 0; j < phdr->numbodyparts; j++ )
	{
		...
		for( k = 0; k < pbodypart->nummodels; k++ )
		{
			...
			ibuffer = pSubModel;
			SWAP_INTS(mstudiomodel_t, unused);
	...
	for( j = 0; j < phdr->numattachments; j++ )
	{
		...
		SWAP_INTS(mstudioattachment_t,flags);
	}
}
#endif
